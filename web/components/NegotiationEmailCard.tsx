...
export function parseNegotiationEmail(text: string): ParsedNegotiationEmail | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("Tactic: ")) tactic = line.slice(8).trim();
      ...
      // First blank line after Subject: marks the start of the body
      bodyStartIdx = i + 1;
    }
  }
...
export function NegotiationEmailCard({ email }: NegotiationEmailCardProps) {
...
{email.body}
      ...
      {/* Psychology toggle — mini-lesson in negotiation */}
      <div className="px-3 pb-2">
        ...
        >
          {showPsychology ? "▾" : "▸"} Why this works
        </button>
        {showPsychology && (
          <p className="text-[10px] text-stone-600 mt-1.5 leading-relaxed bg-violet-50/50 rounded-lg p-2 border border-violet-100">
            {email.psychology}
          </p>
        )}
      </div>

      {/* Actions: copy + open in email client */}
      <div className="px-3 py-2.5 border-t border-stone-100 bg-stone-50/50 flex items-center gap-2">
        <button
          onClick={handleCopy}
        ...
        {mailtoHref && (
          <a
            href={mailtoHref}
            className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-sky-600 text-white hover:bg-sky-700 transition-colors btn-press"
          >
