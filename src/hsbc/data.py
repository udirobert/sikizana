...
def load_ulb_dataset(path: str | Path) -> pd.DataFrame:
    ...
    if not classes.issubset({0, 1}):
        raise DatasetValidationError("Class must contain only binary labels 0 and 1")
    if len(classes) != 2:
        raise DatasetValidationError("Dataset must contain both legitimate and fraud labels")
