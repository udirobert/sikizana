...
# Session plumbing lives in tools/session.py so every tool module shares one
# ContextVar; metric snapshots and trend analysis live in tools/metric_snapshots.py.
from src.tools.session import current_session, set_current_session, svc as _svc  # noqa: E402, F401
from src.tools.metric_snapshots import (  # noqa: E402, F401
...
def get_sector_benchmarks(sector: str = "") -> str:
    ...
    if not sector or sector not in _SECTOR_BENCHMARKS:
        ...
        try:
            ...
            stored = get_session_pref(current_session(), "sector")
        except Exception:  # noqa: BLE001
            stored = None
