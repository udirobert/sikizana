...
def find_duplicate_bills(bills: list[PayableBill]) -> list[ApFinding]:
    ...
    for (supplier_id, number), group in exact_groups.items():
        ...
        evidence = tuple(
            Evidence(
                bill.id,
                bill.invoice_number or "Unnumbered bill",
        ...
        findings.append(
            ApFinding(
                id=stable_id(
                    "ap-duplicate-bill", supplier_id, number, *(bill.id for bill in ordered)
                ),
    ...
    for bill in bills:
        if bill.id in covered_bill_ids or not normalized(bill.reference):
            continue
        candidate_groups[
            (bill.supplier_id, normalized(bill.reference), round(bill.total * 100))
        ].append(bill)
    for (supplier_id, reference, _amount), group in candidate_groups.items():
        ...
        for left, right in zip(ordered, ordered[1:]):
            ...
            if (
                gap is None
                or gap > 7
                or normalized(left.invoice_number) == normalized(right.invoice_number)
            ):
                continue
            findings.append(
                ApFinding(
                    id=stable_id(
                        "ap-near-duplicate-bill", supplier_id, reference, left.id, right.id
                    ),
                    kind="ap_duplicate_bill",
                    severity="medium",
                    ...
                    evidence=(
                        Evidence(
                            left.id,
                            left.invoice_number or "First bill",
                            f"{left.date} · £{left.total:,.2f}",
                        ),
                        Evidence(
                            right.id,
                            right.invoice_number or "Second bill",
                            f"{right.date} · £{right.total:,.2f}",
                        ),
                    ),
                )
            )
